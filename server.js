...
async function searchCharacterSources(character) {
  const query=[character.name,character.series,character.requested_variant,'canonical appearance outfit body proportions'].filter(Boolean).join(' ');
  try{
...
function normalizeVerifiedCharacter(reference) {
  const next={...reference},outfit=String(next.default_outfit||'').trim();
  const first=outfit.match(/(?:^|\s)first outfit\s*:\s*([\s\S]*?)(?=\s*(?:second|third) outfit\s*:|$)/i);
  if(first)next.default_outfit=first[1].trim().replace(/[.;,\s]+$/,'');
  return next;
}
...
async function verifyCharacterReferences(characters) {
  ...
  for(const character of drafts){const sources=await searchCharacterSources(character);evidence.push({name:character.name,series:character.series||'',requested_variant:character.requested_variant||'',sources})}
  if(!evidence.some(x=>x.sources.length))return drafts.map(x=>({name:x.name,series:x.series||'',requested_variant:x.requested_variant||'',appearance:'',body_proportions:'',default_outfit:'',identity_exclusions:'',style:'',confidence:'unverified; no public reference found'}));
  const request={model:llmModel,temperature:0,max_tokens:1400,messages:[
    {role:'system',content:'Create canonical visual identity briefs using ONLY the supplied public reference extracts. Return JSON only: {"characters":[{"name":"canonical name","series":"originating work","requested_variant":"requested arc, era, season, transformation, or named outfit","appearance":"permanent facial identity only: face shape, eyes, eyebrows, nose, mouth, skin, markings, and invariant traits; exclude hair","version_specific_hair":"exact hair color, length, bangs, and styling for the requested version only","body_proportions":"canonical height/build/silhouette stated neutrally","default_outfit":"one specific canonical outfit and colors for the requested version","identity_exclusions":"traits from other characters, eras, forms, or outfits that must not appear","style":"originating series visual style","confidence":"source verified|partial","sources":["source URLs"]}]}. Do not use unsupported memory. IDENTITY ANCHORS: always retain source-supported invariant facial geometry—face shape, eye shape/color, eyebrows, nose, mouth, markings, and other distinguishing features—even when the requested version changes hair or clothing. A correct outfit on a generic face is not canon accuracy. HAIR LOCK: appearance must not contain hair. Put only the requested version hairstyle in version_specific_hair; never combine an earlier/later hairstyle, bangs, or cut with the requested version. VERSION LOCK: when requested_variant is non-empty, extract that arc/era/form specifically and do not substitute the generic, pre-timeskip, post-timeskip, debut, or latest design. If the requested arc contains several outfits and the user did not name a sub-arc, raid, festival, disguise, or outfit number, choose the first/main iconic appearance in that arc and state it precisely. Distinguish everyday human form from transformations, dragon/monster forms, disguises, costumes, and alternate outfits. Never transfer wings, tails, hair colors, clothes, hairstyles, or accessories from another character or version. If a detail is not supported, leave it empty. Do not add rating or adult-content tags.'},
    {role:'user',content:JSON.stringify({characters:drafts.map(x=>({name:x.name,series:x.series||'',requested_variant:x.requested_variant||''})),reference_evidence:evidence})}
  ]};
  if(/deepseek\.com/i.test(llmUrl)){request.thinking={type:'disabled'};request.response_format={type:'json_object'}}
  try{const response=await fetch(llmUrl+'/v1/chat/completions',{method:'POST',headers:{'content-type':'application/json',...(llmApiKey?{authorization:`Bearer ${llmApiKey}`}:{})},body:JSON.stringify(request)}),data=await response.json();if(!response.ok)throw new Error();const parsed=JSON.parse(String(data.choices?.[0]?.message?.content||'{}').replace(/^```(?:json)?\s*|\s*```$/g,''));return Array.isArray(parsed.characters)&&parsed.characters.length?parsed.characters.map(normalizeVerifiedCharacter):drafts}catch{return drafts.map(x=>({name:x.name,series:x.series||'',requested_variant:x.requested_variant||'',appearance:'',body_proportions:'',default_outfit:'',identity_exclusions:'',style:'',confidence:'unverified; reference extraction failed'}))}
}
...
async function resolveCharacterReferences(idea) {
  ...
  const request={model:llmModel,temperature:0.1,max_tokens:900,messages:[
    {role:'system',content:'Extract established named fictional characters from an image idea and prepare an identity lookup request. Return JSON only: {"characters":[{"name":"canonical name","series":"originating work","requested_variant":"exact requested arc, era, season, transformation, disguise, or named outfit; empty if none","appearance":"memory draft","body_proportions":"memory draft","default_outfit":"memory draft","identity_exclusions":"memory draft","style":"memory draft","confidence":"high|medium|low"}]}. Include only established public fictional characters, not generic people or private/custom characters. Preserve every explicitly requested arc or version in requested_variant; never collapse an arc-specific request into a generic post-timeskip or default design. Separate character version from scene intent such as romantic, comedic, glamorous, or fan-service posing. Never invent a setting or outfit merely from a character name. Do not add rating or adult-content tags. If unsure, leave uncertain details empty rather than guessing. Do not debate or add prose.'},
...
module.exports={sanitizeNegativePrompt,inferLoraTrigger,promptDialectForModel,promptGuidanceForModel,matchCharacterCanon,mediaWikiApiUrl,normalizeVerifiedCharacter};
